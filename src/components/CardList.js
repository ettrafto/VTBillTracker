...
import data from "../bills_data.json";
...
const CardList = () => {
  ...
  const [cardsPerRow, setCardsPerRow] = useState(3); // State to control number of cards per row
  const [isDarkMode, setIsDarkMode] = useState(false); // State to manage dark mode
  const location = useLocation();
  const { selectedDistricts } = location.state || {};
